CREATE TABLE public.consecutive_firmware_upgrade_failures (
  ...
  CONSTRAINT fk_rsu_id FOREIGN KEY (rsu_id) REFERENCES rsus(rsu_id) ON DELETE CASCADE
);
...
CREATE TABLE public.email_type (
  ...
  CONSTRAINT fk_role_id FOREIGN KEY (required_role) REFERENCES roles(role_id)
);
...
CREATE TABLE public.firmware_upgrade_rules (
  ...
  CONSTRAINT firmware_upgrade_rules_from_to_unique UNIQUE (from_id, to_id),
...
CREATE TABLE public.intersection_organization (
  ...
  CONSTRAINT fk_intersection_id FOREIGN KEY (intersection_id) REFERENCES intersections(intersection_id) ON DELETE CASCADE,
  CONSTRAINT fk_organization_id FOREIGN KEY (organization_id) REFERENCES organizations(organization_id) ON DELETE RESTRICT,
  CONSTRAINT intersection_organization_pkey PRIMARY KEY (intersection_organization_id),
  CONSTRAINT intersection_organization_unique UNIQUE (intersection_id, organization_id)
);
...
CREATE TABLE public.intersections (
  ...
  CONSTRAINT intersection_number_numeric CHECK (((intersection_number)::text ~ '^[0-9]+$'::text)),
...
CREATE TABLE public.roles (
  ...
  CONSTRAINT roles_name_allowed CHECK (((name)::text = ANY ((ARRAY['admin'::character varying, 'operator'::character varying, 'user'::character varying])::text[]))),
...
CREATE TABLE public.rsu_organization (
  ...
  CONSTRAINT rsu_organization_unique UNIQUE (rsu_id, organization_id)
);
...
CREATE TABLE public.rsus (
  ...
  CONSTRAINT rsus_milepost_non_negative CHECK ((milepost >= (0)::double precision))
);
...
CREATE TABLE public.users (
  ...
  CONSTRAINT users_keycloak_id UNIQUE (keycloak_id),
