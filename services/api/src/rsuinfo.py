...
def get_rsu_data(user: EnvironWithOrg, qualified_orgs: list[str]):
    ...
    query = (
        ...
        "FROM ("
        "SELECT rd.rsu_id, rd.geography, rd.milepost, rd.ipv4_address, rd.serial_number, rd.primary_route, opt.tim_deposit, opt.snmp_monitoring, rm.name AS model_name, man.name AS manufacturer_name "
