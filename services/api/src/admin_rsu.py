...
def get_rsu_data(rsu_ip):
    ...
    query = (
        ...
        f"JOIN {schema_name}.rsu_models AS rm ON rm.rsu_model_id = rsus.model "
        f"JOIN {schema_name}.manufacturers AS man ON man.manufacturer_id = rm.manufacturer "
        f"JOIN {schema_name}.rsu_credentials AS rsu_cred ON rsu_cred.credential_id = rsus.credential_id "
        f"JOIN {schema_name}.snmp_credentials AS snmp_cred ON snmp_cred.snmp_credential_id = rsus.snmp_credential_id "
        f"JOIN {schema_name}.snmp_protocols AS snmp_ver ON snmp_ver.snmp_protocol_id = rsus.snmp_protocol_id "
        f"JOIN {schema_name}.rsu_organization AS ro ON ro.rsu_id = rsus.rsu_id  "
...
def modify_rsu(rsu_spec):
    ...
    try:
        ...
        query = (
            ...
            f"model=(SELECT rsu_model_id FROM {schema_name}.rsu_models WHERE name = '{model}'), "
            f"credential_id=(SELECT credential_id FROM {schema_name}.rsu_credentials WHERE nickname = '{rsu_spec['ssh_credential_group']}'), "
            f"snmp_credential_id=(SELECT snmp_credential_id FROM {schema_name}.snmp_credentials WHERE nickname = '{rsu_spec['snmp_credential_group']}'), "
            f"snmp_protocol_id=(SELECT snmp_protocol_id FROM {schema_name}.snmp_protocols WHERE nickname = '{rsu_spec['snmp_version_group']}'), "
        ...
        for organization in rsu_spec["organizations_to_remove"]:
            org_remove_query = (
                ...
                f"rsu_id=(SELECT rsu_id FROM {schema_name}.rsus WHERE ipv4_address = '{rsu_spec['ip']}') "
...
def delete_rsu(rsu_ip):
    ...
    org_remove_query = (
        f"DELETE FROM {schema_name}.rsu_organization WHERE "
        f"rsu_id=(SELECT rsu_id FROM {schema_name}.rsus WHERE ipv4_address = '{rsu_ip}')"
    )
    pgquery.write_db(org_remove_query)

    # Delete recorded RSU ping data
    ping_remove_query = (
        f"DELETE FROM {schema_name}.ping WHERE "
        f"rsu_id=(SELECT rsu_id FROM {schema_name}.rsus WHERE ipv4_address = '{rsu_ip}')"
    )
    ...
    scms_remove_query = (
        f"DELETE FROM {schema_name}.scms_health WHERE "
        f"rsu_id=(SELECT rsu_id FROM {schema_name}.rsus WHERE ipv4_address = '{rsu_ip}')"
    )
    ...
    msg_config_remove_query = (
        f"DELETE FROM {schema_name}.snmp_msgfwd_config WHERE "
        f"rsu_id=(SELECT rsu_id FROM {schema_name}.rsus WHERE ipv4_address = '{rsu_ip}')"
    )
    pgquery.write_db(msg_config_remove_query)

    # Delete RSU data
    rsu_remove_query = f"DELETE FROM {schema_name}.rsus WHERE ipv4_address = '{rsu_ip}'"
