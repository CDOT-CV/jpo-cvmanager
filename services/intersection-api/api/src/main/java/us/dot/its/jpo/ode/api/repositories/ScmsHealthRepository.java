...
public interface ScmsHealthRepository extends JpaRepository<ScmsHealth, Integer> {
     ...
     *     <li>Using a <b>correlated subquery</b> (sh.timestamp = MAX(sh2.timestamp)) to select only the most recent health record per RSU (equivalent to Python's ROW_NUMBER() window function).</li>
