...
public class OrganizationManagementService {
    ...
    public OrganizationDto modifyOrganization(OrganizationPatch patch, CvManagerAuthToken authToken) {
        List<Integer> authorizedOrgs = authToken.getQualifiedOrgList(UserRole.ADMIN);

        // Step 1: Authorization guard — the caller must be ADMIN in the target org
        if (!authToken.isSuperUser() && !authorizedOrgs.contains(patch.getId())) {
        ...
        if (patch.getTimDeposit() != null || patch.getSnmpMonitoring() != null) {
            applyBulkRsuOptions(org.getId(), patch.getTimDeposit(), patch.getSnmpMonitoring());
        }

        // Step 4: Add users
        handleUsersToAdd(patch.getUsersToAdd(), org.getId(), authorizedOrgs, authToken.isSuperUser());

        // Step 5: Modify user roles
        handleUsersToModify(patch.getUsersToModify(), org.getId());

        // Step 6: Remove users
        if (patch.getUsersToRemove() != null && !patch.getUsersToRemove().isEmpty()) {
            userOrganizationRepository.deleteByUserEmailsAndOrganizationId(
                    patch.getUsersToRemove(), org.getId());
            log.debug("Removed {} user(s) from org '{}'", patch.getUsersToRemove().size(), org.getName());
        }

        // Step 7: Add RSU associations
        handleRsusToAdd(patch.getRsusToAdd(), org.getId());
        ...
        if (patch.getRsusToRemove() != null && !patch.getRsusToRemove().isEmpty()) {
            ...
            rsuOrganizationRepository.deleteByRsuIpv4AddressesAndOrganizationId(addresses, org.getId());
            log.debug("Removed {} RSU(s) from org '{}'", addresses.size(), org.getName());
        }

        // Step 9: Add intersection associations
        handleIntersectionsToAdd(patch.getIntersectionsToAdd(), org.getId());
        ...
        if (patch.getIntersectionsToRemove() != null && !patch.getIntersectionsToRemove().isEmpty()) {
            List<String> numberStrings = patch.getIntersectionsToRemove().stream()
                    .map(Object::toString)
                    .toList();
            intersectionOrganizationRepository.deleteByIntersectionNumbersAndOrganizationId(
                    numberStrings, org.getId());
    ...
    public void deleteOrganization(Integer orgId) {
        ...
        if (rsuOrganizationRepository.existsOrphanRsuInOrganization(orgId)) {
        ...
        if (intersectionOrganizationRepository.existsOrphanIntersectionInOrganization(orgId)) {
            throw new OrganizationHasDependentsException(
                    "Cannot delete organization that has one or more Intersections only associated with this organization");
        }

        if (userOrganizationRepository.existsOrphanUserInOrganization(orgId)) {
            throw new OrganizationHasDependentsException(
                    "Cannot delete organization that has one or more users only associated with this organization");
        }

        userOrganizationRepository.deleteAllByOrganizationId(orgId);
        rsuOrganizationRepository.deleteAllByOrganizationId(orgId);
        intersectionOrganizationRepository.deleteAllByOrganizationId(orgId);
        organizationRepository.delete(org);
        log.debug("Organization '{}' deleted", orgId);
    }
    ...
    private void applyBulkRsuOptions(Integer orgId, Boolean timDeposit, Boolean snmpMonitoring) {
        List<InetAddress> rsuIps = rsuOrganizationRepository.findAllRsuIpsByOrganizationId(orgId);
        if (rsuIps.isEmpty()) {
            return;
        }
        ...
        log.debug("Bulk-applied RSU options to {} RSU(s) in org '{}'", optionsToSave.size(), orgId);
    }

    private void handleUsersToAdd(List<UserRoleAssignment> assignments, Integer orgId,
            List<Integer> authorizedOrgs, boolean isSuperUser) {
        if (assignments == null || assignments.isEmpty()) {
            return;
        }

        // Non-superusers may only add users to organizations they administer
        if (!isSuperUser && !authorizedOrgs.contains(orgId)) {
            throw new ResponseStatusException(HttpStatus.FORBIDDEN,
                    "User does not have permission to add users to organization: " + orgId);
        }
        ...
        for (UserRoleAssignment assignment : assignments) {
            ...
            boolean alreadyMember = userOrganizationRepository
                    .findByUser_EmailAndOrganization_Id(assignment.getEmail(), orgId)
                    .isPresent();
            if (alreadyMember) {
                log.debug("User '{}' is already a member of org '{}', skipping add", assignment.getEmail(), orgId);
                continue;
            }
            ...
            log.debug("Queued user '{}' with role '{}' for addition to org '{}'", assignment.getEmail(),
                    assignment.getRole(), orgId);
        }
        userOrganizationRepository.saveAll(toSave);
    }

    private void handleUsersToModify(List<UserRoleAssignment> assignments, Integer orgId) {
        if (assignments == null || assignments.isEmpty()) {
            return;
        }
        ...
        for (UserRoleAssignment assignment : assignments) {
            UserOrganization userOrg = userOrganizationRepository
                    .findByUser_EmailAndOrganization_Id(assignment.getEmail(), orgId)
                    .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND,
                            "User '" + assignment.getEmail() + "' is not a member of organization: " + orgId));
    ...
    private void handleRsusToAdd(List<String> ipStrings, Integer orgId) {
        ...
        for (String ipString : ipStrings) {
            ...
            boolean alreadyAssigned = rsuOrganizationRepository
                    .findByRsuIpv4AddressAndOrganization_Id(ip, orgId)
                    .isPresent();
            if (alreadyAssigned) {
                log.debug("RSU '{}' is already assigned to org '{}', skipping add", ipString, orgId);
                continue;
            }
            ...
            toSave.add(rsuOrg);
            log.debug("Queued RSU '{}' for addition to org '{}'", ipString, orgId);
        }
        rsuOrganizationRepository.saveAll(toSave);
    }

    private void handleIntersectionsToAdd(List<Integer> intersectionIds, Integer orgId) {
        ...
        for (Integer intersectionId : intersectionIds) {
            ...
            boolean alreadyAssigned = intersectionOrganizationRepository
                    .findByIntersection_IntersectionNumberAndOrganization_Id(idString, orgId)
                    .isPresent();
            if (alreadyAssigned) {
                log.debug("Intersection '{}' is already assigned to org '{}', skipping add", intersectionId, orgId);
                continue;
            }
            ...
            toSave.add(io);
            log.debug("Queued intersection '{}' for addition to org '{}'", intersectionId, orgId);
        }
