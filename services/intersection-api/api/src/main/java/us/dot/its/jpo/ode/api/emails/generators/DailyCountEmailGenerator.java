...
public class DailyCountEmailGenerator extends AbstractEmailGenerator<DailyCountEmailContents> {
    ...
    @Override
    public EmailContent generateEmailBody(DailyCountEmailContents data) {
    ...
    private String generateEmailHtml(DailyCountEmailContents data) {
        ...
        html.append(String.format("<h2>%s %s Count Report %s UTC - %s UTC</h2>",
                escapeHtml(data.getOrganizationName()), escapeHtml(data.getDeploymentTitle()), startDate, endDate));
        ...
        for (Map.Entry<String, List<MessageCount>> entry : data.getRsuCountsByOrganization().entrySet()) {
            ...
            if (!counts.isEmpty()) {
                html.append(generateCountTable(counts));
            }
        }
    ...
    private String generateCountTable(List<MessageCount> counts) {
        StringBuilder html = new StringBuilder();
        ...
        for (Map.Entry<String, List<MessageCount>> entry : countsByRsu.entrySet()) {
            ...
            html.append(String.format("<td style=\"border: 1px solid #ddd;\">%s</td>", escapeHtml(rsuIp)));

            String road = rsuCounts.isEmpty() ? "Unknown" : rsuCounts.get(0).getRoad();
            html.append(String.format("<td style=\"border: 1px solid #ddd;\">%s</td>", escapeHtml(road)));
