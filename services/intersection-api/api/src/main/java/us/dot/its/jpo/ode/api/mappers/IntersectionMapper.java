...
/**
 * MapStruct mapper for converting Intersection entities to/from Intersection
 * DTOs.
 *
 * The {@code rsus} field is intentionally excluded (mapped to ignore) because
 * RSU IPs
 * are fetched via a separate query in the service and set manually after
 * mapping.
 * This avoids loading the rsuIntersections lazy collection during entity-to-DTO
 * conversion.
 */
@Mapper(componentModel = MappingConstants.ComponentModel.SPRING, uses = { GeometryMapper.class, INetMapper.class })
public interface IntersectionMapper {
    ...
    @Mapping(source = "intersectionOrganizations", target = "organizations", qualifiedByName = "mapOrgIds")
