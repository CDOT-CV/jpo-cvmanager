...
class UpdatePostgresSnmpAbstractClass(ABC):
    def get_rsu_list(self):
        ...
        query = (
            ...
            "FROM {schema_name}.rsus AS rd "
            "LEFT JOIN {schema_name}.snmp_credentials AS snmp ON snmp.snmp_credential_id = rd.snmp_credential_id "
            "LEFT JOIN {schema_name}.snmp_protocols AS sver ON sver.snmp_protocol_id = rd.snmp_protocol_id"
            ") as row"
        )
