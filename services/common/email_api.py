...
class EmailApi:
    ...
    def get_kc_token(self) -> KeycloakToken | None:
        ...
        if status_code == 200:
            self.token = token
            self.token_expiration_date = datetime.datetime.now() + datetime.timedelta(milliseconds=token["expires_in"])
        else:
    ...
    def send_message_counts(
        ...
        response = requests.post(f"{self.iapi_endpoint}/emails/send-message-counts", headers={'Authorization': f"bearer {token}"}, json={
            "org_name": org_name,
        ...
        if not (200 <= response.status_code < 300):
            logging.error(f"Failed to send message counts email: {response.status_code} - {response.text}")
        return response.status_code, response.json()
    ...
    def send_firmware_upgrade_failure(
        ...
        """
        token = self.get_kc_token()
        response = requests.post(f"{self.iapi_endpoint}/emails/send-firmware-upgrade-failure", headers={'Authorization': f"bearer {token}"}, json={
            "rsu_ip": rsu_ip,
    ...
    def send_rsu_error_summary(
        ...
        """
        token = self.get_kc_token()
        response = requests.post(f"{self.iapi_endpoint}/emails/send-rsu-error-summary", headers={'Authorization': f"bearer {token}"}, json={
