...
def test_delete_config_list(mock_write_db):
    ...
    mock_write_db.assert_has_calls(
        [
            call(
                "DELETE FROM {schema_name}.snmp_msgfwd_config WHERE rsu_id=1 AND msgfwd_type=2 AND snmp_index=1"
            ),
            call(
                "DELETE FROM {schema_name}.snmp_msgfwd_config WHERE rsu_id=2 AND msgfwd_type=3 AND snmp_index=1"
            ),
        ]
    )
