...
def insert_rsu_ping_mqtt(request_json):
    global db
    if db is None:
        db = init_tcp_connection_engine()
    ...
    with db.connect() as conn:
        ...
        timestamp = request_json["timestamp"] / 1000
        logging.debug(
            f"rsu_id: {rsu_id}, result: {result}, timestamp: {timestamp}"
        )
        if rsu_id and result and timestamp:
            logging.debug(
                f"Inserting new Ping record for RsuData {rsu_id}"
            )
            try:
                query = f"INSERT INTO public.ping (timestamp, result, rsu_id) VALUES (to_timestamp({timestamp}), B'{result}', {rsu_id})"
                logging.debug(query)
                conn.execute(sqlalchemy.text(query))
                conn.commit()
            except Exception as e:
