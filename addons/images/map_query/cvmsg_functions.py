...
def map_intersection_geometry_to_geojson(ip, intersectionGeometry):
    ...
    for lane in intersectionGeometry.get('laneSet', {}).get('GenericLane', []):
        ...
        props = {
            ...
            'ingressPath': "true" if lane.get('laneAttributes', {}).get('directionalUse', {}).get('ingressPath', False) else "false",
            'egressPath': "true" if lane.get('laneAttributes', {}).get('directionalUse', {}).get('egressPath', False) else "false",
            'egressApproach': lane.get('egressApproach', 0),
            'ingressApproach': lane.get('ingressApproach', 0),
            'ip': ip,
            'connectedLanes': [conn['connectingLane']['lane'] for conn in lane.get('connectsTo', {}).get('connectsTo', [])]
        }
            ...
            for deltatype, deltavalue in node['delta'].items():
                if deltatype == 'nodeLatLon':
                    ...
                    if deltavalue.get('x') is not None:
                        offset_lon_lat = node_xy_to_lon_lat(reference_point=(anchor_lon, anchor_lat),
                                                            x=deltavalue['x'], y=deltavalue['y'])
