...
import pytest
from images.psm_query import psm_query

from images.psm_query.psm_query import create_message, process_message, run
...
@patch.dict(
    os.environ,
    {
        ...
        "MONGO_PSM_INPUT_COLLECTION": "psm_input",
        "MONGO_GEO_OUTPUT_COLLECTION": "geo_output",
    },
)
@patch("images.psm_query.psm_query.ThreadPoolExecutor")
def test_run(mock_thread_pool_executor, mock_mongo_client):
    mock_collection = mock_mongo_client.__getitem__.return_value
    psm_query.set_mongo_client = MagicMock(
