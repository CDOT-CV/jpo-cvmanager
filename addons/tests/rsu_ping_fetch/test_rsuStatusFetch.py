...
def test_setZabbixAuth():
    ...
    # check
    rsuStatusFetch.logging.info.assert_called_once_with(
        "Fetching Zabbix auth token from endpoint"
    )
    rsuStatusFetch.requests.post.assert_called_once_with(
        "endpoint",
        json={
            "jsonrpc": "2.0",
            "method": "user.login",
...
def test_run_insert_failure():
    ...
    rsf.getHistory.assert_called_once()
    rsf.insertHistoryItem.assert_called_once()
    rsuStatusFetch.logging.warning.assert_called_once_with(
...
def test_run_exception():
    ...
    rsf.getHostInfo.assert_called_once()
    rsuStatusFetch.logging.warning.assert_not_called()
    rsuStatusFetch.logging.error.assert_called_once_with(
