...
const DisplayRsuErrors = ({ initialSelectedRsu }: { initialSelectedRsu?: RsuInfo }) => {
  ...
  const getRSUSCMSStatus = (rsuIpv4: string): boolean => {
            ...
            <Accordion elevation={0}>
              ...
              <AccordionDetails>
                <div>
                  <Typography fontSize="small" sx={{ color: theme.palette.text.secondary }}>
                    <b>SCMS Status: </b>
                    {getRSUSCMSStatus(selectedRSU.properties.ipv4_address) ? 'Healthy' : 'Unhealthy'}
                  </Typography>
                  <Typography fontSize="small" sx={{ color: theme.palette.text.secondary }}>
                    <b>SCMS Expiration: </b>
                  ...
                  render: (rowData) => (
                    <>
                      <Typography
                        fontSize="medium"
                        sx={{
                          color: rowData.scms_status ? theme.palette.success.dark : theme.palette.error.dark,
                          fontWeight: 'bold',
                        }}
                      >
                        {rowData.scms_status ? 'Healthy' : 'Unhealthy'}
                      </Typography>
            ...
            <MaterialTable
              ...
              data={
                rsuTableData !== undefined
                  ? rsuTableData.filter((row) => row.online_status.includes('Offline') || !row.scms_status)
                  : []
              }
              title=""
              options={{
